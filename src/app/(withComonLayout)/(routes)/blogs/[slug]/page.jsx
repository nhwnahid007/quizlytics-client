...
export default function Post({ params }) {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await axios.get('https://quizlytics.jonomukti.org/allBlogs'); // Fetch all posts
        const posts = response.data; // Assuming the API returns an array of posts
        const foundPost = posts.find(p => p.slug === slug); // Find the post with the matching slug

        if (foundPost) {
          setPost(foundPost);
          // Filter out the current post to display other posts
          setOtherPosts(posts.filter(p => p.slug !== slug).slice(0, 3)); // Display up to 3 other posts
        } else {
  ...
  return (
    <div className="flex justify-around items-start mt-10 container mx-auto gap-10">
      {/* Blog Post Section */}
...
<div className="grid grid-cols-1 md:grid-cols-3  gap-6">
  {otherPosts.map((otherPost) => (
    <div key={otherPost.slug} className="bg-white p-4 rounded-lg shadow-lg">
      <Image 
        src={otherPost.photo} 
        alt={otherPost.title} 
        width={400} 
      ...
      />
      <h3 className="text-lg font-semibold text-primary-color">{otherPost.title}</h3>
      <p className="text-sm text-gray-500">{otherPost.summary}</p>
...
<div className="text-center mt-8">
  <Link href="/blogs">
    <button className="px-4 py-2 bg-primary-color text-white font-medium rounded-lg hover:bg-primary-dark transition duration-300">
      ...
      {/* Article Links for AI Questions Section */}
      <div className='mt-32'>
